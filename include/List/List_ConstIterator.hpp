...
/// @details 양방향 반복자(Bidirectional Iterator) 이다.\n
/// 기본적으로 이 상수 반복자가 가리키는 리스트와 리스트 내부 노드를 포인터(\ref
/// m_node_ptr, \ref m_cont_ptr)로 저장하고 있다.
...
public: // IO Iterator
    ...
    auto operator*() const noexcept -> const Val_t&;
...
public: // Forward Iterator
    ...
    /// 비정상 종료하는지 확인.
    auto operator++() noexcept -> List_ConstIterator&;
    /// @brief 이 상수 반복자가 가리키는 리스트 노드를 리스트의 다음 노드로
    /// 바꾼다.
    ...
    /// @test 반복자가 현재 센티넬 노드를 가리키고 있을 때, 이 연산을 수행하면
    /// 비정상 종료하는지 확인.
    auto operator++(int) noexcept -> List_ConstIterator;
...
private:
    /// @brief 인자로 전달된 리스트에 대한 포인터가 이 상수 반복자가 가리키는
    /// 리스트와 호환되는지 확인한다.
    /// @param cont_ptr 이 반복자와 호환성을 확인할 리스트
    /// @return 호환성 여부. 호환이 되는 경우 `true`, 그렇지 않으면 `false`
    /// @details 매개 변수가 포인터로 전달되기 때문에 항상 유효한 리스트를
    /// 가리키지 않을 수도 있음을 주의한다.
    auto IsCompatible_unchecked(const List_t* cont_ptr) const -> bool;
...
template <class List_t>
List_ConstIterator<List_t>::List_ConstIterator(
    const List_t* cont_ptr, const Node_D_t* node_pos_ptr) noexcept
...
auto List_ConstIterator<List_t>::operator->() const noexcept -> const Val_t*
{
    return std::pointer_traits<const typename List_t::Val_t*>::pointer_to(
        operator*());
}

template <typename List_t>
auto List_ConstIterator<List_t>::operator++() noexcept
...
template <typename List_t>
auto List_ConstIterator<List_t>::operator++(int) noexcept
    -> List_ConstIterator<List_t>
{
    // 현재 노드가 센티넬 노드가 아닌 경우에만 증가 연산을 수행한다.
    RDS_Assert(m_data_ptr != m_cont_ptr->GetSentinelPointer() &&
...
{
    return IsCompatible_unchecked(other.m_cont_ptr) &&
...
{
    return IsValid() && m_cont_ptr->GetSentinelPointer() != m_data_ptr;
}
...
template <class List_t>
auto List_ConstIterator<List_t>::IsCompatible_unchecked(
    const List_t* cont_ptr) const -> bool
{
    return cont_ptr == m_cont_ptr;
}

template <class List_t>
auto List_ConstIterator<List_t>::GetDataPointer() const -> const Node_D_t*
{
    return m_data_ptr;
}
