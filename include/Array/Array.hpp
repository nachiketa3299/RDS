...
/// @details 단순한 원시 배열에 대한 래퍼로 구현되어 있다. 초기값을 전달하지
/// 않는 경우 default-initialization 하기 때문에, `T_t`에 대한 기본 생성자가
/// 정의되어 있으야 한다.
...
public: // Default CDtors
    /// @brief 기본 생성자. @ref Val_t 에 대한 기본 생성자가 정의되어 있어야
    /// 하는 이유이다.
    Array()                   = default;
    /// @brief 기본 복사 생성자
...
public: // Element Access - Position
    /// @brief 인자로 전달된 인덱스의 배열 원소에 대한 const-lvalue 참조를
    /// 반환한다.
    /// @details 배열 아래 첨자 연산자에 대한 오버로딩이다. 인덱스의 범위 검사를
    /// 하지 않는다.
    ...
    /// @brief 인자로 전달된 인덱스의 배열 원소에 대한 const-lvalue 참조를
    /// 반환한다.
    /// @details 배열의 범위를 검사하여 유효한 인덱스인 경우에만 참조를
    /// 반환한다. 유효하지 않은 경우 비정상 종료한다.
    /// @test \ref Array_ElementAccess_gtest.cpp 에서 테스트.
    /// @test `index` 가 [0, `Size_v`) 범위에 속하지 않는 경우, 비정상
    /// 종료하는지 확인.
...
public: // Element Access - Front & Back
    ...
    /// @details 내부적으로 이 배열을 상수 배열로 형변환한 후, \ref Front()
    /// const 함수를 호출하여 반환값을 `const_cast` 하는 방식으로 작동한다.
    auto Front() -> Val_t&;
    ...
    /// @return 배열의 맨 첫번째 원소에 대한 lvalue 참조
    /// @details 내부적으로 이 배열을 상수 배열로 형변환한 후, \ref Back() const
    /// 함수를 호출하여 반환값을 `const_cast` 하는 방식으로 작동한다.
    auto Back() -> Val_t&;
...
public: // Capacity
    ...
    /// @brief 배열의 최대 크기(원소의 갯수)를 반환한다. 정적 배열이므로 배열의
    /// 크기와 같은 값을 반환한다.
    /// @return 배열의 최대 크기
...
{
    return const_cast<T_t&>(
        static_cast<const Array<T_t, Size_v>&>(*this).Front());
}
...
{
    return const_cast<T_t&>(
        static_cast<const Array<T_t, Size_v>&>(*this).Back());
}
